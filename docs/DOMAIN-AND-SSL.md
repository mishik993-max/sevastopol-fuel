...
После оплаты домен появится в личном кабинете. Запишите его: например `sevazs.su`.
...
**Вариант А — сайт на корне домена** (`https://sevazs.su`):

| Тип | Имя (хост) | Значение | TTL |
|-----|------------|----------|-----|
| A | `@` | `185.12.34.56` | 300–3600 |

**Вариант Б — поддомен** (`https://fuel.sevazs.su`):
...
- В поле «Имя» у некоторых панелей `@` пишут как пустое поле или сам домен `sevazs.su`.
...
```nginx
server_name sevazs.su;   # или fuel.sevazs.su
```

В `.env`:

```env
APP_URL=https://sevazs.su
```
...
# или с www:
sudo certbot --nginx -d sevazs.su -d www.sevazs.su
```
...
| Домен | Почему удобно |
|-------|----------------|
| **sevazs.su** | Севастополь + АЗС, 7 символов до точки |
...
**Рекомендация:** если свободен — **`sevazs.su`** или **`zaprav.su`**: коротко, по-русски понятно, хорошо смотрится в ссылке `https://sevazs.su`.
